...
| Componente Azure | Função | Componente AWS | Diferença Operacional |
|---|---|---|---|
| 7 empresas (Atalaia, Fredini, Ecofeed, Mellore, Purazul, FictorBetim, FictorPrados) | Tenants reais do grupo | **5 tenants anonimizados** (`unit_01..unit_05`) | Fictor removido; nomes substituídos |
| Schema por empresa em Bronze (`bronze__srv_infra_bigdata_dw__pub_atalaia`) | Isolamento físico | **Particionamento por `tenant_id` em S3 + tabela única** | Mais simples; evita 35 tabelas separadas |
| `id_agrosys` como source key | ID do ERP origem | **`id_erp_internal`** | Sanitizado — Agrosys removido |
